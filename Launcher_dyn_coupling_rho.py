...
lat_freez      = min(max(np.arccos(max(min((T_freez-T_av+5*pi)/20,1),-1))*180/pi,0),90)
...
print(T_av, lat_freez, rho, T_surf)
...
if rho1 > 0:
	while t[-1] < tmax:
		...
		if t[-1] < 2000:
			dt   = max(min(abs(y0[no_zero]/np.array(der)[no_zero]))/10,1e-2)
		else:
			#dt   = min(abs(y0[no_zero]/np.array(der)[no_zero]))/10
			dt   = max(min(abs(y0[no_zero]/np.array(der)[no_zero]))/5,1e-1)
		...
		lat_freez1 = min(max(np.arccos(max(min((T_freez-T_av1+5*pi)/20,1),-1))*180/pi,0),90)
